...
def api_recrutiment_trainig():
  if request.form.get('action') == 'confirmed' or request.form.get('action') == 'draft':
    ...
    if request.form.get('action') == 'confirmed':
      confirm_recruitment_task(recruitment.id)
...
def confirm_recruitment_task(recruitment_id):
  from app.utils.background_jobs import confirmRecruitment
  
  confirm  = confirmRecruitment(recruitment_id)
  
  thr = Thread(target=confirm.confirm_recruitment())
  thr.start()
  return thr
