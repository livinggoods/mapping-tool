...
def register():
    ...
    # Username is autogenerated from the email
    if form.validate_on_submit():
        username = form.email.data.split('@')[0]
        geo = Geo.query.filter_by(id=form.location.data).first()
        user = User(email=form.email.data,
                    username=username,
                    app_name=form.password.data.encode('base64'),
                    name=form.name.data,
                    password=form.password.data,
                    location=geo.geo_code,
                    geo_id=form.location.data)
        db.session.add(user)
        db.session.commit()  # must force commit to generate user id
        token = user.generate_confirmation_token()
        send_email(user.email, 'Confirm Your Account',
